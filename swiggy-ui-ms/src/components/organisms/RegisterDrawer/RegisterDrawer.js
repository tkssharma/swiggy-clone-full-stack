...
function OtpDrawer({ phoneNumber, name, email, password, setState, state }) {
	...
	const handleVerify = () => {
		...
		axios
			.post(`${process.env.REACT_APP_API_URL}/api/customer/register/verify`, {
				otp: otp,
				name: name,
				email: email,
				password: password,
	...
	const list = (anchor) => (
		...
		>
			<Div className="container mt-3" style={{ width: '90%' }}>
				<Div className="row">
					<Div className="col text-left">
						<button type="button" className="btn btn-sm" onClick={toggleOTPDrawer(anchor, false)}>
	...
	return (
		<div>
			...
			<Drawer anchor={'right'} open={state2['right']} onClose={toggleOTPDrawer('right', false)}>
				{list('right')}
			</Drawer>
		</div>
	);
