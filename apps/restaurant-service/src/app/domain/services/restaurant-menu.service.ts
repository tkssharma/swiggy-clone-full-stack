...
import {
  CreateRestaurantMenuParam,
  CreateRestaurantParam,
  RestaurantMenuParamById,
  RestaurantParamById,
...
export default class RestaurantMenuService {
  ...
  public async create(
    params: RestaurantParamById,
    data: CreateRestaurantMenuParam
  ): Promise<Menu> {
    try {
      ...
      if (!restaurant) {
        throw new NotFoundException(
          `restaurant with uuid ${params.id} not found`
        );
      }
  ...
  public async getMenuById(
    params: RestaurantMenuParamById
